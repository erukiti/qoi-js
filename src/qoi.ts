...
const write1 = (buf: Buf, n: number) => {
  buf.arr[buf.ptr++] = n;
};

const write2 = (buf: Buf, n: number) => {
  buf.arr[buf.ptr++] = (n >> 8) & 0xff;
  buf.arr[buf.ptr++] = n & 0xff;
};

const write3 = (buf: Buf, n: number) => {
  buf.arr[buf.ptr++] = (n >> 16) & 0xff;
  buf.arr[buf.ptr++] = (n >> 8) & 0xff;
  buf.arr[buf.ptr++] = n & 0xff;
};

const write4 = (buf: Buf, n: number) => {
...
const writeChar = (buf: Buf, c: string) => {
...
export const createBuf = (
  ...
  return {
    arr: new Uint8Array(getMaxLength(width, height, channels)),
    ptr: 0,
  };
};
...
export const encode = (
  ...
  const buf = createBuf(width, height, channels);
